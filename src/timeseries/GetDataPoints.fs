...
type GetDataPointsClientExtensions =
    ...
    /// <returns>A single datapoint response object containing a list of datapoints.</returns>
    [<Extension>]
    static member GetAsync (this: ClientExtensions.DataPoints, id : int64, options: DataPointQuery seq, [<Optional>] token: CancellationToken) : Task<DataPointListResponse> =
        async {
    ...
    /// <returns>List of datapoint responses containing lists of datapoints for each timeseries.</returns>
    [<Extension>]
    static member ListMultipleAsync (this: ClientExtensions.DataPoints, options: DataPointMultipleQuery seq, defaultOptions: DataPointQuery seq, [<Optional>] token: CancellationToken) : Task<DataPointListResponse> =
        async {
