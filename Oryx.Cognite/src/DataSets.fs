...
module DataSets =
    ...
    let create (items: DataSetCreate seq) (source: HttpHandler<unit>) : HttpHandler<DataSet seq> =
        source |> withLogMessage "DataSets:create" |> create items Url
    ...
    let list (query: DataSetQuery) (source: HttpHandler<unit>) : HttpHandler<ItemsWithCursor<#DataSet>> =
        source |> withLogMessage "DataSets:list" |> list query Url
    ...
    let aggregate (query: DataSetQuery) (source: HttpHandler<unit>) : HttpHandler<int> =
        source |> withLogMessage "DataSets:aggregate" |> aggregate query Url
    ...
    let update (query: IEnumerable<UpdateItem<DataSet>>) (source: HttpHandler<unit>) : HttpHandler<#DataSet seq> =
        source |> withLogMessage "DataSets:update" |> update query Url
