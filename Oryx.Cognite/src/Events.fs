...
module Events =
    ...
    let get (eventId: int64) : HttpHandler<unit, #Event, 'TResult> =
    ...
    let list (query: EventQuery) : HttpHandler<unit, ItemsWithCursor<#Event>, 'TResult> =
    ...
    let aggregate (query: EventQuery) : HttpHandler<unit, int32, 'TResult> =
    ...
    let create (items: EventCreate seq) : HttpHandler<unit, #Event seq, 'TResult> =
    ...
    let delete (items: EventDelete) : HttpHandler<unit, EmptyResponse, 'TResult> =
    ...
    let retrieve (ids: Identity seq) (ignoreUnknownIds: Nullable<bool>) : HttpHandler<unit, #Event seq, 'TResult> =
    ...
    let search (query: EventSearch) : HttpHandler<unit, #Event seq, 'TResult> =
    ...
    let update (query: UpdateItem<EventUpdate> seq) : HttpHandler<unit, #Event seq, 'TResult>  =
