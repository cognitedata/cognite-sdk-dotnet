...
module ThreeDModels =
    ...
    let list (query: ThreeDModelQuery) : IHttpHandler<unit, ItemsWithCursor<ThreeDModel>> =
    ...
    let create (items: ThreeDModelCreate seq) : IHttpHandler<unit, ThreeDModel seq> =
    ...
    let delete (ids: IEnumerable<Identity>) : IHttpHandler<unit, EmptyResponse> =
    ...
    let retrieve (modelId: int64) : IHttpHandler<unit, ThreeDModel> =
        let url = Url
    ...
    let update (query: IEnumerable<UpdateItem<ThreeDModelUpdate>>) : IHttpHandler<unit, ThreeDModel seq>  =
...
module ThreeDRevisions =
    ...
    let list (modelId: int64) (query: ThreeDRevisionQuery) : IHttpHandler<unit, ItemsWithCursor<ThreeDRevision>> =
    ...
    let create (modelId: int64) (items: ThreeDRevisionCreate seq) : IHttpHandler<unit, ThreeDRevision seq> =
        let url = Url +/ sprintf "%d" modelId +/ "revisions"
    ...
    /// <returns>Empty result.</returns>
    let delete (modelId: int64) (ids: IEnumerable<Identity>) : IHttpHandler<unit, EmptyResponse> =
        let url = Url +/ sprintf "%d" modelId +/ "revisions"
        let items = ItemsWithoutCursor(Items=ids)
    ...
    let retrieve (modelId: int64) (revisionId: int64) : IHttpHandler<unit, ThreeDRevision> =
        let url = Url +/ sprintf "%d" modelId +/ "revisions"
    ...
    let update (modelId: int64) (query: IEnumerable<UpdateItem<ThreeDRevisionUpdate>>) : IHttpHandler<unit, ThreeDRevision seq>  =
        let url = Url +/ sprintf "%d" modelId +/ "revisions"
    ...
    /// <param name="query">The update query.</param>
    /// <returns>Corresponing revisions after applying the updates.</returns>
    let updateThumbnail (modelId: int64) (revisionId: int64) (fileId: int64) : IHttpHandler<unit, EmptyResponse>  =
    ...
    /// <returns>List of 3DRevisions matching given filters and optional cursor</returns>
    let listLogs (modelId: int64) (revisionId: int64) (query: ThreeDRevisionLogQuery) : IHttpHandler<unit, ItemsWithCursor<ThreeDRevisionLog>> =
...
module ThreeDNodes =
    ...
    let list (modelId: int64) (revisionId: int64) (query: ThreeDNodeQuery) : IHttpHandler<unit, ItemsWithCursor<ThreeDNode>> =
...
module ThreeDAssetMappings =
    ...
    let list (modelId: int64) (revisionId: int64) (query: ThreeDAssetMappingQuery) : IHttpHandler<unit, ItemsWithCursor<ThreeDAssetMapping>> =
    ...
    let create (modelId: int64) (revisionId: int64) (items: ThreeDAssetMappingCreate seq) : IHttpHandler<unit, ThreeDAssetMapping seq> =
        let url = Url +/ sprintf "%d" modelId +/ "revisions" +/ sprintf "%d" revisionId +/ "mappings"
    ...
    /// <returns>Empty result.</returns>
    let delete (modelId: int64) (revisionId: int64) (assetMappings: IEnumerable<Identity>) : IHttpHandler<unit, EmptyResponse> =
        let url = Url +/ sprintf "%d" modelId +/ "revisions" +/ sprintf "%d" revisionId +/ "mappings"
