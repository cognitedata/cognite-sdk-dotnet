...
module TemplateGroups =
    ...
    let baseVersionedUrl externalId version = Url +/ externalId +/ "versions" +/ (version |> string)

    let instancesUrl externalId version =  baseVersionedUrl externalId version +/ "instances"

    let viewsUrl externalId version = baseVersionedUrl externalId version +/ "views"
    ...
    let retrieve (items: string seq) (ignoreUnknownIds: Nullable<bool>) (source: HttpHandler<unit>) : HttpHandler<TemplateGroup seq> =
        source
        |> withLogMessage "templategroups:retrieve"
        |> retrieveIgnoreUnknownIds (items |> Seq.map (fun id -> Identity.Create id)) (Option.ofNullable ignoreUnknownIds) Url
    ...
    let upsertVersion (externalId: string) (item: TemplateVersionCreate) (source: HttpHandler<unit>) : HttpHandler<TemplateVersion> =
        source
    ...
    let filterVersions (externalId: string) (query: TemplateVersionQuery) (source: HttpHandler<unit>) : HttpHandler<ItemsWithCursor<TemplateVersion>> =
        source
    ...
    let createInstances (externalId: string) (version: int) (items: TemplateInstanceCreate seq) (source: HttpHandler<unit>) : HttpHandler<TemplateInstance seq> =
        source
    ...
    let upsertInstances (externalId: string) (version: int) (items: TemplateInstanceCreate seq) (source: HttpHandler<unit>) : HttpHandler<TemplateInstance seq> =
        source
        |> withLogMessage "templateinstances:create"
        |> Handler.create items ((instancesUrl externalId version) +/ "upsert")

    /// Update instances for the given template group version.
    let updateInstances (externalId: string) (version: int) (items: UpdateItem<TemplateInstanceUpdate> seq) (source: HttpHandler<unit>) : HttpHandler<TemplateInstance seq> =
        source
    ...
    let retrieveInstances (externalId: string) (version: int) (items: string seq) (ignoreUnknownIds: Nullable<bool>) (source: HttpHandler<unit>) : HttpHandler<TemplateInstance seq> =
        source
        |> withLogMessage "templateinstances:retrieve"
        |> retrieveIgnoreUnknownIds (items |> Seq.map (fun id -> Identity.Create id)) (Option.ofNullable ignoreUnknownIds) (instancesUrl externalId version)
    ...
    let filterInstances (externalId: string) (version: int) (query: TemplateInstanceFilterQuery) (source: HttpHandler<unit>) : HttpHandler<ItemsWithCursor<TemplateInstance>> =
        source
    ...
    let deleteInstances (externalId: string) (version: int) (items: string seq) (ignoreUnknownIds: bool) (source: HttpHandler<unit>) : HttpHandler<unit> =
        let query = ItemsWithIgnoreUnknownIds(Items=(items |> Seq.map (fun id -> CogniteExternalId(id))), IgnoreUnknownIds=ignoreUnknownIds)
        source
    ...
    let deleteViews (externalId: string) (version: int) (items: string seq) (ignoreUnknownIds: bool) (source: HttpHandler<unit>) : HttpHandler<unit> =
        let query = ItemsWithIgnoreUnknownIds(Items=(items |> Seq.map (fun id -> CogniteExternalId(id))), IgnoreUnknownIds=ignoreUnknownIds)
        source
        |> withLogMessage "templateinstances:delete"
