...
module Files =
    ...
    let get (fileId: int64) : HttpHandler<unit, File, 'a> =
    ...
    let list (query: FileQuery) : HttpHandler<unit, ItemsWithCursor<File>, 'a> =
    ...
    let aggregate (query: FileQuery) : HttpHandler<unit, int32, 'a> =
        withLogMessage "Files:aggregate"
        >=> aggregate query Url

    /// Upload new file in the given project.
    let upload (file: FileCreate) (overwrite: bool) : HttpHandler<unit, FileUploadRead, 'a> =
        withLogMessage "Files:upload"
        >=> postV10 file Url

    /// Get download URL for file in the given project.
    let download (ids: Identity seq) : HttpHandler<unit, IEnumerable<FileDownload>, 'a> =
    ...
    let retrieve (ids: Identity seq) (ignoreUnknownIds: Nullable<bool>) : HttpHandler<unit, IEnumerable<File>, 'a> =
        withLogMessage "Files:retrieve"
        >=> retrieveIgnoreUnkownIds ids (Option.ofNullable ignoreUnknownIds) Url

    let search (query: FileSearch) : HttpHandler<unit, File seq, 'a> =
        withLogMessage "Files:search"
        >=> search query Url

    let delete (files: ItemsWithoutCursor<Identity>) : HttpHandler<unit, EmptyResponse, 'a> =
    ...
    let update (query: IEnumerable<UpdateItem<FileUpdate>>) : HttpHandler<unit, File seq, 'a>  =
