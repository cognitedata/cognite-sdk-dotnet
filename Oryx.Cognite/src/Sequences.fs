...
module Sequences =
    ...
    let list (query: SequenceQuery) : HttpHandler<unit, ItemsWithCursor<Sequence>, 'a> =
    ...
    let listRows (query: SequenceRowQuery) : HttpHandler<unit, SequenceData, 'a> =
        withLogMessage "Sequences:listRows"
        >=> Handler.list query RowsUrl

    /// Retrieves number of sequences matching filter.
    let aggregate (query: SequenceQuery): HttpHandler<unit, int32, 'a> =
        withLogMessage "Sequences:aggregate"
        >=> aggregate query Url

    /// Create new sequences in the given project. Returns list of created sequences.
    let create (items: SequenceCreate seq) : HttpHandler<unit, SequenceData seq, 'a> =
        withLogMessage "Sequences:create"
        >=> create items Url

    let createRows(items: SequenceDataCreate seq) : HttpHandler<unit, EmptyResponse, 'a> =
    ...
    let delete (items: Identity seq) : HttpHandler<unit, EmptyResponse, 'a> =
        let req = ItemsWithoutCursor(Items=items)
        withLogMessage "Sequences:delete"
        >=> delete req Url

    let deleteRows (items: SequenceRowDelete seq) : HttpHandler<unit, EmptyResponse, 'a> =
        let req = ItemsWithoutCursor(Items=items)
    ...
    let retrieve (ids: Identity seq) (ignoreUnknownIds: Nullable<bool>) : HttpHandler<unit, Sequence seq, 'a> =
    ...
    let search (query: SequenceSearch) : HttpHandler<unit, Sequence seq, 'a> =
    ...
    let update (query: IEnumerable<UpdateItem<SequenceUpdate>>) : HttpHandler<unit, Sequence seq, 'a>  =
