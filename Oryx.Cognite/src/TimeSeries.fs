...
module TimeSeries =
    ...
    let list (query: TimeSeriesQuery) (source: HttpHandler<unit>) : HttpHandler<ItemsWithCursor<#TimeSeries>> =
        source |> withLogMessage "TimeSeries:get" |> list query Url
    ...
    let aggregate (query: TimeSeriesQuery) (source: HttpHandler<unit>) : HttpHandler<int32> =
        source |> withLogMessage "TimeSeries:aggregate" |> aggregate query Url
    ...
    let search (query: TimeSeriesSearch) (source: HttpHandler<unit>) : HttpHandler<#TimeSeries seq> =
        source |> withLogMessage "TimeSeries:search" |> search query Url
    ...
    let update
        ...
        (source: HttpHandler<unit>)
        : HttpHandler<#TimeSeries seq> =
        source |> withLogMessage "TimeSeries:update" |> update query Url
