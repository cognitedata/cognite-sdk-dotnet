...
module TimeSeries =
    ...
    let get (tsId: int64) : HttpHandler<unit, #TimeSeries> =
    ...
    let list (query: TimeSeriesQuery) : HttpHandler<unit, ItemsWithCursor<#TimeSeries>> =
        withLogMessage "TimeSeries:get"
        >=> list query Url

    /// Retrieves number of time series matching filter. Returns number of time
    /// series matching given filters.
    let aggregate (query: TimeSeriesQuery) : HttpHandler<unit, int32> =
        withLogMessage "TimeSeries:aggregate"
        >=> aggregate query Url

    /// Create one or more new time series. Returns list of created time series.
    let create (items: IEnumerable<TimeSeriesCreate>) : HttpHandler<unit, IEnumerable<#TimeSeries>> =
        withLogMessage "TimeSeries:create"
        >=> create items Url

    /// Delete one or more time series.
    let delete (items: TimeSeriesDelete) : HttpHandler<unit, EmptyResponse> =
    ...
    let retrieve (ids: Identity seq) (ignoreUnknownIds: Nullable<bool>) : HttpHandler<unit, #TimeSeries seq> =
    ...
    let search (query: TimeSeriesSearch) : HttpHandler<unit, #TimeSeries seq> =
    ...
    let update (query: IEnumerable<UpdateItem<TimeSeriesUpdate>>) : HttpHandler<unit, #TimeSeries seq>  =
    ...
    let syntheticQuery (query: TimeSeriesSyntheticQuery) : HttpHandler<unit, IEnumerable<DataPointsSyntheticItem>> =
